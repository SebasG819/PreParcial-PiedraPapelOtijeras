...
function draw() {
    background(0); // Fondo negro para que el texto blanco sea visible
    ...
    // Ajuste de la posición Y para las imágenes del enemigo
...
async function MandarDatos(selection) {
    socket.emit('send-element1', selection);
    console.log("element 1 send", selection);
    ...
    await (socket.on('element2-received', (selecc) => {
        console.log("received-element:", selecc);
...
socket.on('element1-received', (selecc) => {
    console.log("received-element:", selecc);
    selection = selecc;
});
